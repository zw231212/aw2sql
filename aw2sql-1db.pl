...
use vars qw/
    $VERSION $DIR $PROG $Extension $SiteConfig $DataDir $MonthConfig $YearConfig $help $database 
...
sub Create_Table
{
  ...
  elsif($_[0] eq "hours")
  {
    $s = "CREATE TABLE  IF NOT EXISTS `hours` ( ".
        ...
        "`hits` MEDIUMINT UNSIGNED NOT NULL , ".
        "`bandwidth` BIGINT UNSIGNED NOT NULL , ".
        "PRIMARY KEY (`config`, `year_month`, `hour` ) );";
  }
  elsif($_[0] eq "session")
  {
    $s = "CREATE TABLE IF NOT EXISTS  `session` ( ".
        ...
        "`visits` MEDIUMINT UNSIGNED NOT NULL , ".
        "PRIMARY KEY (`config`, `year_month`, `range` ) );";
  }
  elsif($_[0] eq "domain")
  {
    $s = "CREATE TABLE IF NOT EXISTS  `domain` ( ".
        ...
        "`hits` MEDIUMINT UNSIGNED NOT NULL , ".
        "`bandwidth` BIGINT UNSIGNED NOT NULL , ".
        "PRIMARY KEY (`config`, `year_month`, `code` ) );";
  }
  elsif($_[0] eq "os")
  {
  ...
  elsif($_[0] eq "filetypes")
  {
    $s = "CREATE TABLE  IF NOT EXISTS `filetypes` ( ".
        ...
        "PRIMARY KEY (`config`, `year_month`, `type` ) );";
  }
  elsif($_[0] eq "screen")
  {
    $s = "CREATE TABLE  IF NOT EXISTS `screen` ( ".
        ...
        "`hits` MEDIUMINT UNSIGNED NOT NULL , ".
        "PRIMARY KEY (`config`, `year_month`, `size` ) );";
  }
  elsif($_[0] eq "misc")
  {
  ...
  elsif($_[0] eq "origin")
  {
    $s = "CREATE TABLE  IF NOT EXISTS `origin` ( ".
        ...
        "`pages` MEDIUMINT UNSIGNED NOT NULL, ".
        "`hits` MEDIUMINT UNSIGNED NOT NULL , ".
        "PRIMARY KEY (`config`,`year_month`, `from` ) );";
  }
...
Read_General();
$sth = $dbh->prepare("SELECT COUNT(*) FROM general WHERE `year_month`='".$general {'year_month'}."' AND `config`='".$SiteConfig."';");
$sth->execute();
my $count = $sth->fetchrow_array();
$sth->finish();

$sql = " `general` SET `year_month`='".$general{'year_month'}."', `config`='".$SiteConfig."', ".
...
elsif($count==1) { $sql = "UPDATE".$sql." WHERE `year_month`='".$general{'year_month'}."' AND `config`='".$SiteConfig."' LIMIT 1;"; }
else { error("There are repeated rows for the date ".$general{'year_month'}." into the 'general' table of ".$SiteConfig."\n"); }
$rows = $dbh->do($sql);
if(!$rows) { error("We can't add a new rows to the 'general' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
...
for (my $i=1; $i<=$maxday; $i++)
{
  ...
  Read_Daily($tempdate);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM daily WHERE `day`='".$daily{'day'}."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();

  $sql = " `daily` SET `day`='".$daily{'day'}."', `visits`='".$daily{'visits'}."', `config`='".$SiteConfig."', ".
      " `pages`='".$daily{'pages'}."', `hits`='".$daily{'hits'}."', ".
      " `bandwidth`='".$daily{'bandwidth'}."'";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `day`='".$daily{'day'}."' AND `config`='".$SiteConfig."'  LIMIT 1;"; }
  else { error("There are repeated rows for the date ".$daily{'day'}." into the 'daily' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new rows to the 'daily' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
{
  Read_Hours($i);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM hours WHERE `year_month`='".$year_month."' AND `hour` = '".$i."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();
  $sql = " `hours` SET `hour`='".$hours{'hour'}."', `pages`='".$hours{'pages'}."',  `config`='".$SiteConfig."', ".
      " `hits`='".$hours{'hits'}."', `bandwidth`='".$hours{'bandwidth'}."', `year_month` = '".$year_month."'";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `year_month`='".$year_month."' AND `hour` = '".$i."' AND `config`='".$SiteConfig."' LIMIT 1;"; }
  else { error("There are repeated hours into the 'hours' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new rows to the 'hours' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
{
  Read_Session($i);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM session WHERE `year_month` = '".$year_month."' AND `range`='".$session{'range'}."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();

  $sql = " `session` SET `range`='".$session{'range'}."', `visits`='".$session{'visits'}."',  `year_month` = '".$year_month."', `config`='".$SiteConfig."' ";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `year_month` = '".$year_month."' AND  `range`='".$session{'range'}."' AND `config`='".$SiteConfig."'  LIMIT 1;"; }
  else { error("There are repeated times into the 'session' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'session' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
{
  Read_Domain($i);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM domain WHERE `year_month` = '".$year_month."' AND `code`='".$domain{'code'}."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();

  $sql = " `domain` SET `code`='".$domain{'code'}."', `pages`='".$domain{'pages'}."', `config`='".$SiteConfig."',  ".
      "`hits`='".$domain{'hits'}."', `bandwidth`='".$domain{'bandwidth'}."' , `year_month` = '".$year_month."'";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `year_month` = '".$year_month."' AND `code`='".$domain{'code'}."' AND `config`='".$SiteConfig."'  LIMIT 1;"; }
  else { error("There are repeated regional codes into the 'domain' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'domain' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
{
  Read_OS($i);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM os WHERE `name`='".$os{'name'}."' AND `year_month`='".$os{'year_month'}."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();

  $sql = " `os` SET `name`='".$os{'name'}."', `year_month`='".$os{'year_month'}."', `hits`='".$os{'hits'}."', `config`='".$SiteConfig."' ";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `name`='".$os{'name'}."' AND `year_month`='".$os{'year_month'}."' AND `config`='".$SiteConfig."'  LIMIT 1;"; }
  else { error("There are repeated OS codes into the 'os' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'os' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `unkos` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Vaciamos la tabla
for (my $i=0; $i<$max; $i++)
{
  Read_unkos($i);

  $sql = "INSERT INTO `unkos` SET `agent`=?, `lastvisit`='".$unkos{'lastvisit'}."',`year_month` = '".$year_month."', `config`='".$SiteConfig."' ;";
  ...
  if(!$rows) { error("We can't add a new row to the 'unkos' table in the".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
{
  Read_Browser($i);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM browser WHERE `name`='".$browser{'name'}."' AND `year_month`='".$browser{'year_month'}."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();

  $sql = " `browser` SET `name`='".$browser{'name'}."', `year_month`='".$browser{'year_month'}."', `hits`='".$browser{'hits'}."', `config`='".$SiteConfig."' ";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `name`='".$browser{'name'}."' AND `year_month`='".$browser{'year_month'}."' AND `config`='".$SiteConfig."'  LIMIT 1;"; }
  else { error("There are repeated browsers' codes into the 'browser' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'browser' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `unkbrowser` WHERE `year_month` ='".$year_month."' AND `config`='".$SiteConfig."' ;"); # Vaciamos la tabla
for (my $i=0; $i<$max; $i++)
{
  Read_unkbrowser($i);

  $sql = "INSERT INTO `unkbrowser` SET `agent`=?,`year_month` = '".$year_month."', `lastvisit`='".$unkbrowser{'lastvisit'}."', `config`='".$SiteConfig."' ;";
  ...
  if(!$rows) {
    print($agentInfo."\n");
    error("We can't add a new row to the 'unkbrowser' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
{
  Read_FileTypes($i);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM filetypes WHERE `year_month`= '".$year_month."' AND `type`='".$ft{'type'}."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();

  $sql = " `filetypes` SET `type`='".$ft{'type'}."', `hits`='".$ft{'hits'}."', `config`='".$SiteConfig."', ".
      "`bandwidth`='".$ft{'bandwidth'}."', `bwwithoutcompress`='".$ft{'bwwithoutcompress'}."', ".
      "`bwaftercompress`='".$ft{'bwaftercompress'}."', `year_month` = '".$year_month."'";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `year_month`= '".$year_month."' AND  `type`='".$ft{'type'}."' AND `config`='".$SiteConfig."'  LIMIT 1;"; }
  else { error("There are repeated file types into the 'filetypes' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'filetypes' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
{
  Read_Screen($i);
  $sth = $dbh->prepare("SELECT COUNT(*) FROM screen WHERE `year_month` = '".$year_month."' AND `size`='".$screen{'size'}."' AND `config`='".$SiteConfig."' ");
  $sth->execute();
  my $count = $sth->fetchrow_array();
  $sth->finish();

  $sql = " `screen` SET `size`='".$screen{'size'}."', `hits`='".$screen{'hits'}."', `year_month` = '".$year_month."', `config`='".$SiteConfig."' ";
  if($count==0) { $sql = "INSERT INTO".$sql.";"; }
  elsif($count==1) { $sql = "UPDATE".$sql." WHERE `year_month` = '".$year_month."' AND  `size`='".$screen{'size'}."' AND `config`='".$SiteConfig."' LIMIT 1;"; }
  else { error("There are repeated screen resolutions into the 'screen' table of ".$SiteConfig."\n"); }
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'screen' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `misc` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
for (my $i=0; $i<$max; $i++)
{
  Read_Misc($i);
  $sql = "INSERT INTO `misc` SET `text`='".$misc{'text'}."', `pages`='".$misc{'pages'}."', `config`='".$SiteConfig."', ".
      " `hits`='".$misc{'hits'}."', `bandwidth`='".$misc{'bandwidth'}."', `year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'misc' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `worms` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
for (my $i=0; $i<$max; $i++)
{
  Read_Misc($i);
  $sql = "INSERT INTO `worms` SET `text`='".$worms{'text'}."', `hits`='".$worms{'hits'}."',`config`='".$SiteConfig."', ".
      "`bandwidth`='".$worms{'bandwidth'}."', `lastvisit`='".$worms{'lastvisit'}."', `year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'worms' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `robot` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
for (my $i=0; $i<$max; $i++)
{
  Read_Robot($i);
  $sql = "INSERT INTO `robot` SET `name`='".$robot{'name'}."', `hits`='".$robot{'hits'}."', `config`='".$SiteConfig."',  ".
      "`bandwidth`='".$robot{'bandwidth'}."', `lastvisit`='".$robot{'lastvisit'}."', `year_month` = '".$year_month."', ".
      "`hitsrobots`='".$robot{'hitsrobots'}."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'robot' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `errors` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
for (my $i=0; $i<$max; $i++)
{
  Read_Errors($i);
  $sql = "INSERT INTO `errors` SET `code`='".$errors{'code'}."', `hits`='".$errors{'hits'}."', `config`='".$SiteConfig."', ".
      "`bandwidth`='".$errors{'bandwidth'}."', `year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'errors' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `errors404` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
for (my $i=0; $i<$max; $i++)
{
  Read_Errors404($i);
  $e404{'url'} =~ tr/'/&#039;/; # we subs the incorrect character ' with its html code
  $sql = "INSERT INTO `errors404` SET `url`='".$e404{'url'}."', `hits`='".$e404{'hits'}."',`config`='".$SiteConfig."',  ".
      "`referer`='".$e404{'referer'}."', `year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new row to the 'errors404' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `visitors` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;");
for (my $i=0; $i<$max; $i++)
{
  Read_Visitors($i);
  $sql = "INSERT INTO `visitors` SET `host`='".$visit{'host'}."', `pages`='".$visit{'pages'}."', `config`='".$SiteConfig."', ".
  ...
  $sql = $sql .";";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new visitor to the 'visitors' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `pages` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;");
for (my $i=0; $i<$max; $i++)
{
  Read_Pages($i);
  ...
  $sql = "INSERT INTO `pages` SET `url`=?, `pages`=?, `config`=?, ".
      "`bandwidth`=?, `entry`=?, `year_month` = ?, ".
      "`exit`=?;";
  my $prepare = $dbh->prepare($sql);
  $rows = $prepare->execute($urlInfo,$pagesNum,$SiteConfig,$bandWidthNum,$entryNum,$year_month,$exitNum)
      or die "We can't add a new entry to the 'pages' table in the ".$database." database.\n $DBI::err ($DBI::errstr)" ;
  if(!$rows) {
    print($urlInfo."\n");
    next;
  }
...
$rows = $dbh->do("DELETE FROM `origin` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;");
for (my $i=0; $i<$max; $i++)
{
  Read_Origin($i);
  $sql = "INSERT INTO `origin` SET `from`='".$origin{'from'}."', `pages`='".$origin{'pages'}."', `config`='".$SiteConfig."', ".
      "`hits`='".$origin{'hits'}."', `year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new entry to the 'origin' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `searchref` WHERE`year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;");
for (my $i=0; $i<$max; $i++)
{
  Read_Searchref($i);
  $sql = "INSERT INTO `searchref` SET `engine`='".$searchref{'engine'}."', `pages`='".$searchref{'pages'}."', `config`='".$SiteConfig."',  ".
      "`hits`='".$searchref{'hits'}."',`year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new entry to the 'searchref' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `pageref` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;");
for (my $i=0; $i<$max; $i++)
{
  Read_Pageref($i);
  $sql = "INSERT INTO `pageref` SET `url`='".$pageref{'url'}."', `pages`='".$pageref{'pages'}."', `config`='".$SiteConfig."', ".
      "`hits`='".$pageref{'hits'}."', `year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new entry to the 'pageref' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `searchwords` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;");
for (my $i=0; $i<$max; $i++)
{
  Read_Searchwords($i);
  $sql = "INSERT INTO `searchwords` SET `words`='".$searchwords{'words'}."', `config`='".$SiteConfig."',  `hits`='".$searchwords{'hits'}."',`year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new entry to the 'searchwords' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `searchkeywords` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;");
for (my $i=0; $i<$max; $i++)
{
  Read_Searchkeywords($i);
  $sql = "INSERT INTO `searchkeywords` SET `words`='".$searchkeywords{'words'}."', `config`='".$SiteConfig."', `hits`='".$searchkeywords{'hits'}."', `year_month` = '".$year_month."';";
  $rows = $dbh->do($sql);
  if(!$rows) { error("We can't add a new entry to the 'searchkeywords' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
$rows = $dbh->do("DELETE FROM `downloads` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
for (my $i=0; $i<$max; $i++)
{
  Read_Downloads($i);
  $sql = "INSERT INTO `downloads` SET `url`=?, `downloads`='".$downloads{'downloads'}."', `config`='".$SiteConfig."',  ".
  ...
  if(!$rows) { error("We can't add a new row to the 'downloads' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
}
...
if ($max > 0) {
  $rows = $dbh->do("DELETE FROM `errors403` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
  for (my $i=0; $i<$max; $i++)
  {
    Read_Errors403($i);
    $e403{'url'} =~ tr/'/&#039;/; # we subs the incorrect character ' with its html code
    $sql = "INSERT INTO `errors403` SET `url`=?, `hits`='".$e403{'hits'}."', `config`='".$SiteConfig."',  ".
        "`referer`='".$e403{'referer'}."', `year_month` = '".$year_month."';";
    my $e403Prepare = $dbh->prepare($sql);
    $rows = $e403Prepare->execute($e403{'url'});
    if(!$rows) { error("We can't add a new row to the 'errors403' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
  }
}
...
if($max > 0){
  $rows = $dbh->do("DELETE FROM `errors400` WHERE `year_month` = '".$year_month."' AND `config`='".$SiteConfig."' ;"); # Empty the table
  for (my $i=0; $i<$max; $i++)
  {
    Read_Errors400($i);
    $e400{'url'} =~ tr/'/&#039;/; # we subs the incorrect character ' with its html code
    $sql = "INSERT INTO `errors400` SET `url`=?, `hits`='".$e400{'hits'}."', `config`='".$SiteConfig."',  ".
        "`referer`='".$e400{'referer'}."', `year_month` = '".$year_month."';";
    my $e400Prepare = $dbh->prepare($sql);
    $rows = $e400Prepare->execute($e400{'url'});
    if(!$rows) { error("We can't add a new row to the 'errors400' table in the ".$database." database.\n $DBI::err ($DBI::errstr)"); }
  }
}
